...
class Requests:
    def __init__(self, logger, uuid):
        self.movie_index = random.randint(1, 999)
        self.user_index = random.randint(1, 999)
        self.username = "username_" + str(self.user_index)
        self.password = "password_" + str(self.user_index)
        self.title = movie_titles[self.movie_index]
        self.rating = random.randint(0, 10)
        self.text = get_random_string(256)
        self.host = "http://10.107.91.27:8080"
    ...
    def compose_review(self):
        ...
        path = "/wrk2-api/review/compose"
        headers = {"Content-Type": "application/x-www-form-urlencoded"}

        body = {"username": self.username, "password": self.password, "title": self.title, "rating": str(self.rating),
                "text": self.text}

        response = requests.post(url=self.host+path, data=body, headers=headers)

        response_time = time.time() - start_time
        to_log = str(arrival_time) + "," + "compose_review," + str(response.status_code) + "," + str(
...
def user_tasks(logger, uuid):
    global total_user
    total_user += 1
    compose_task = ["compose_review"]
    req_obj = Requests(logger, uuid)

    for task in compose_task:
